...
export default {
  ...
  methods: {
    ...
    clearAdded() {
      this.added.forEach(item => item.isAdded = false);
      this.totalSum = 0;
      this.added = [];
      this.saveAddedToLocalStorage();
    },
...
<template>
  <DrawerApp v-if="openDrawer" :closeDrawerFunc="closeDrawerFunc" :added="added" :fetchAdded="fetchAdded" :totalSum="totalSum" />
