...
import { fetchSmartfones } from "./services/mockApi.js";
...
export default {
  ...
  methods: {
    ...
    async loadSmartfones() {
      try {
        const smartfones = await fetchSmartfones({
        ...
        console.error("Не удалось загрузить список устройств:", error);
      }
    },
    fetchFavorites(id) {
      for (let i = 0; i < this.smartfones.length; i += 1) {
        if (this.smartfones[i]._id === id) {
          this.smartfones[i].isFavorite = !this.smartfones[i].isFavorite;
          if (this.smartfones[i].isFavorite) {
...
<template>
  ...
  <div class="w-11/12 m-auto bg-white rounded-xl shadow-xl mt-10 md:w-5/6 lg:w-11/12">
    ...
    <div class="p-10">
      <div class="flex flex-col items-center" v-if="!isBasketPage && !isFavoritePage">
        <div><h2 class="text-3xl font-bold">Каталог</h2></div>
        <div class="flex justify-center gap-4 flex-wrap py-4">
          <div class="items-center">
            <select class="py-2 px-3 border rounded-md outline-none" v-model="category">
              <option class="sm:text-xs" value="">Все бренды</option>
          ...
          <div class="relative">
            <img class="absolute left-5 top-3" src="/search.svg" alt="поиск">
            <input
              class="border rounded-md py-2 pl-10 pr-4 outline-none focus:border-gray-400 pl-12"
              type="text"
          ...
          <div>
            <button
              @click="loadSmartfones"
              class="bg-lime-600 border px-8 py-2 rounded-md hover:bg-lime-700 transition text-white"
            >
              Применить фильтры
            </button>
          </div>
  ...
  <div class="text-center p-10 mb-10" v-if="$route.path === '/'">
    <button
      @click="goBasket"
      class="bg-lime-600 border px-8 py-2 rounded-md hover:bg-lime-700 transition text-white text-lg"
    >
      Перейти в корзину
    </button>
  </div>
